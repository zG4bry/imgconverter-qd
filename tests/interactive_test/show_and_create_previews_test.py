...
def test_show_and_create_previews_full_success(
    mock_convert_image, mock_get_file_size, capsys
):

    mock_get_file_size.return_value = "test KB"
    mock_convert_image.return_value = "mocked_ok"
    result = show_and_create_previews(img, filename, output_dir)
    expected_output = {
...
def test_show_and_create_previews_fail(mock_convert_image, capsys):
    mock_convert_image.return_value = None
    show_and_create_previews(img, filename, output_dir)
